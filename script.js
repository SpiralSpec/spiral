...
const songs = [
    ...
    // Add song2.mp3 to web folder to enable Song 2
];
...
function pickRandomSong() {
    ...
    if (backgroundMusic) {
        ...
        // Try next available song
        const availableSongs = songs.filter(song => document.getElementById(song.id));
        if (availableSongs.length > 0) {
            const fallbackSong = availableSongs[0];
            backgroundMusic = document.getElementById(fallbackSong.id);
            if (backgroundMusic) {
...
// Dynamic Status Bar System - CUSTOMIZABLE
function initializeDynamicStatus() {
    const statusBar = document.getElementById('dynamic-status');
    if (!statusBar) return;
    
    // ===== CUSTOMIZE THESE SETTINGS =====
    const config = {
        // Add/remove status keywords here
        keywords: [
            'if you couldnt tell but im spiral',
            'welcome', 
            'im new to the apk modding so thanks for greeting me',
            'uh i like fishing',
            'i code you can click the results button on what i know this status might dissapear',
            'uh idk if the music works rn',
            'idk what else to say lol',
            'view counter is very delayed sowwy',
            'uh thats all',
        ],
        
        // Timing settings (in milliseconds)
        typewriterSpeed: 50,        // Speed of typing (lower = faster)
        displayDuration: 3000,     // How long each status shows
        fadeDuration: 200,         // Fade transition speed
        initialDelay: 1000         // Delay before first status
    };
    // ===== END CUSTOMIZABLE SETTINGS =====
    
    let currentIndex = 0;
    
    // Function to update status with typewriter effect
    function updateStatus() {
        const keyword = config.keywords[currentIndex];
        ...
        setTimeout(() => {
            ...
            const typeInterval = setInterval(() => {
                if (charIndex < keyword.length) {
                    statusBar.textContent += keyword[charIndex];
                    charIndex++;
                } else {
                    clearInterval(typeInterval);
                    
                    // Schedule next status change
                    setTimeout(updateStatus, config.displayDuration);
                }
            }, config.typewriterSpeed);
        }, config.fadeDuration);
        
        currentIndex = (currentIndex + 1) % config.keywords.length;
    }
    
    // Start the status updates
    setTimeout(updateStatus, config.initialDelay);
    
    // Make config globally accessible
    window.statusConfig = config;
}
...
function initializeControls() {
    ...
    resultsButton.addEventListener('click', function() {
        if (!showingResults) {
            ...
            // Change status to away when viewing results
            setDiscordStatus('away');
        } else {
            hideResults();
            this.textContent = 'View Results';
            showingResults = false;
            
            // Change status back to online when returning to profile
    ...
    profileCard.innerHTML = `
        ...
        <div class="stats">
            <div class="view-count">
                ...
                <span id="visitor-count">204.392</span>
            </div>
        </div>
    `;
